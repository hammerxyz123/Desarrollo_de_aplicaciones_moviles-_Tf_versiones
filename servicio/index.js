...
app.post('/register', (req, res) => {
    const { datos, email, password, direcion, celular } = req.body;
  
    const query = 'INSERT INTO usuarios (datos, email, password, direcion, celular) VALUES (?, ?, ?, ?, ?)';
  
   conexion.query(query, [datos, email, password, direcion, celular], (err, result) => {
      if (err) {
        return res.status(500).send(err);
      }
      res.status(200).json({ message: 'Usuario registrado exitosamente', result });
    });
  });
...
app.post("/festividad/agregar", (req, res) =>{
    const festividades = {
        ...
        fes_latitud:req.body.fes_latitud,
        fes_longitud:req.body.fes_longitud
    }
...
app.put("/festividad/actualizar/:id",(req,res)=>{
    const {id}=req.params
    const {fes_nombre, fes_descripcion,fes_abono,fes_fecha,fes_observacion,fes_latitud,fes_longitud}=req.body
    const query="UPDATE festividad SET fes_nombre= '"
    +fes_nombre+"', fes_descripcion='"+fes_descripcion+"', fes_abono='"+fes_abono+
    "', fes_fecha='"+fes_fecha+"', fes_observacion='"+fes_observacion+"', fes_latitud="+ 
    fes_latitud+", fes_longitud="+fes_longitud+" WHERE fes_id="+id+";"
    console.log(query)
    conexion.query(query,(error)=>{
        if(error)
            return console.error(error.message)
